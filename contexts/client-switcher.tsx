...
export function ClientSwitcher() {
  const { loadDynamicTheme, isDynamicTheme, dynamicTheme, theme, setTheme } = useTheme()
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem onClick={() => handleClientSelect("client-1")}>
          <span
            className={`mr-2 h-2 w-2 rounded-full bg-sky-400 ${isDynamicTheme && dynamicTheme?.id === "client-1" ? "ring-2 ring-sky-400 ring-offset-2" : ""}`}
          ></span>
          <span>Cliente 1 - Ocean Blue</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => handleClientSelect("client-2")}>
          <span
            className={`mr-2 h-2 w-2 rounded-full bg-orange-400 ${isDynamicTheme && dynamicTheme?.id === "client-2" ? "ring-2 ring-orange-400 ring-offset-2" : ""}`}
          ></span>
